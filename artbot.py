...
def find_external_link(soup, current_url: str) -> str:
    """لینک خارجی برگزارکننده رو پیدا می‌کنه."""
    current_domain = current_url.split("/")[2]
    skip_domains = ["google", "facebook", "instagram", "twitter", "linkedin", "youtube", "t.co"]
    # اول دنبال لینک با کلمات کلیدی
    for a in soup.find_all("a", href=True):
        href = a["href"]
        ...
        if not href.startswith("http"):
            continue
        parts = href.split("/")
        link_domain = parts[2] if len(parts) > 2 else ""
        if link_domain == current_domain or any(s in link_domain for s in skip_domains):
            continue
        if any(w in text for w in ["apply", "apply here", "website", "official site", "submit", "learn more"]):
            return href
    # بعد هر لینک خارجی
    for a in soup.find_all("a", href=True):
        href = a["href"]
        ...
        parts = href.split("/")
        link_domain = parts[2] if len(parts) > 2 else ""
        if link_domain and link_domain != current_domain and not any(s in link_domain for s in skip_domains):
            return href
    return ""
...
def parse_single_opportunity(url: str) -> dict:
    ...
    try:
        ...
        external_link = find_external_link(soup, url)
        resp = client.messages.create(
            model="claude-haiku-4-5-20251001",
            max_tokens=800,
            system="""Extract art opportunity info. Return ONLY valid JSON (no markdown):
{"title":"...","org":"actual organizer name NOT the platform","deadline":"May 15, 2026 or empty string","country":"or empty string","description":"1-2 sentences about the opportunity"}
org must be the actual organizing institution, not ArtJobs/NYFA/platform names.""",
            messages=[{"role":"user","content":f"URL: {url}\n\n{page_text}"}]
        )
        text_resp = resp.content[0].text.strip().replace("```json","").replace("```","").strip()
        data = json.loads(text_resp)
        data["url"] = external_link or url
        return data
    except Exception as e:
