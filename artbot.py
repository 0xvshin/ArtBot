...
ARTCONNECT_OPENCALLS = "https://www.artconnect.com/opportunities/opencalls"
...
def fetch_opportunity_urls() -> list:
    """لیست لینک‌های فراخوان رو از ArtConnect می‌گیره."""
    try:
        r = requests.get(ARTCONNECT_OPENCALLS, headers=HEADERS, timeout=20)
        ...
        for a in soup.find_all("a", href=True):
            ...
            if "/opportunity/" in href:
                ...
                if full not in seen and len(a.get_text(strip=True)) > 5:
                    seen.add(full)
                    urls.append(full)
        return urls[:15]
...
def parse_opportunity_with_claude(artconnect_url: str) -> dict:
    """از Claude می‌خواد صفحه فراخوان رو بخونه و اطلاعات رو استخراج کنه."""
    try:
...
Important: org should be the actual organizer, not ArtConnect.""",
        ...
        # لینک نهایی: website اگه داشت، وگرنه لینک آرت‌کانکت
...
Given an art opportunity JSON, return ONLY valid JSON (no markdown):
{
  ...
  "deadline_fa": "Translate only the month name to Persian equivalent, keep numbers as Persian digits. 'April 29, 2026' → '۲۹ آوریل ۲۰۲۶'",
